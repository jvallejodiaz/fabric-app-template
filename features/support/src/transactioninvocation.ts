...
export class TransactionInvocation {
  ...
  getResult(): string {
    if (!this.result) {
      throw new Error(
        `No transaction result. Error is: ${toString(this.error)}`,
      );
    }
  ...
  getError(): Error {
    if (!this.error) {
      throw new Error(
        `No transaction error. Result is: ${bytesAsString(this.result)}`,
      );
    }
  ...
  private async evaluate(): Promise<Uint8Array> {
    ...
    proposal = await this.sign(
      proposal,
      this.gateway.newSignedProposal.bind(this.gateway),
    );
  ...
  private async submit(): Promise<Uint8Array> {
    ...
    const signedProposal = await this.sign(
      unsignedProposal,
      this.gateway.newSignedProposal.bind(this.gateway),
    );
    ...
    const signedCommit = await this.sign(
      submitted,
      this.gateway.newSignedCommit.bind(this.gateway),
    );
