...
export class Fabric {
  ...
  async createChannels(): Promise<void> {
    ...
    for (const ord of orderers) {
      const orddir =
        "/etc/hyperledger/configtx/crypto-config/ordererOrganizations/example.com/orderers/" +
        ord.address;
      dockerCommand(
        "exec",
        "org1_cli",
  ...
  async deployChaincode(
    ...
    const ccPath = `/opt/gopath/src/github.com/chaincode/${ccName}`;
    ...
    const collectionsFile = `chaincode/${ccName}/collections_config.json`;
